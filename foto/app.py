import sqlite3
import requests
from foto.estados.rj import rj_consulta
#from foto.estados.es import es_consulta
...
def login():
    ...
    'X-Authorization': 'Basic ZmFiaWFuby5jb21pbkBob3RtYWlsLmNvbTpGYWJpYW5vQDIwMjI',
...
'''def cpf_uf(cpf, token):
    ...
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/113.0.0.0 Safari/537.36 OPR/99.0.0.0',
        ...
        if '"code":200' in response:
            uf = GetStr(response, 'siglaUf":"', '"')
            nome = GetStr(response, 'nome":"', '"')
            mae = GetStr(response, 'nomeMae":"', '"')

            if uf == 'RJ':
                return rj_consulta(nome, mae)
            if uf == 'ES':
                return es_consulta(nome, mae)
